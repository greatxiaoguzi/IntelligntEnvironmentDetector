...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueWet != cur->valueWet)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueWet Changed\n");
    ...
    if(last->valueHCHO != cur->valueHCHO)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueHCHO Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    ...
    if(0 != memcmp((uint8_t *)&last->valuePM,(uint8_t *)&cur->valuePM,sizeof(last->valuePM)))
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuePM Changed");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(0 != memcmp((uint8_t *)&last->valueKLW,(uint8_t *)&cur->valueKLW,sizeof(last->valueKLW)))
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueKLW Changed");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueWet = gizY2X(Wet_RATIO,  Wet_ADDITION, dataPoints->valueWet); 

    devStatusPtr->valueTemp = exchangeBytes(gizY2XFloat(Temp_RATIO,  Temp_ADDITION, dataPoints->valueTemp));     
    devStatusPtr->valueHCHO = exchangeBytes(gizY2X(HCHO_RATIO,  HCHO_ADDITION, dataPoints->valueHCHO)); 
    devStatusPtr->valueCO2 = exchangeBytes(gizY2X(CO2_RATIO,  CO2_ADDITION, dataPoints->valueCO2)); 



    gizMemcpy((uint8_t *)devStatusPtr->valuePM,(uint8_t *)&dataPoints->valuePM,sizeof(dataPoints->valuePM));
    gizMemcpy((uint8_t *)devStatusPtr->valueKLW,(uint8_t *)&dataPoints->valueKLW,sizeof(dataPoints->valueKLW));
    return 0;
}
